...
run_online()
{
        ...
        echo
        echo "================================================================"
        su - ${DB2USR} -c "db2 backup database ${SID} online use TSM open ${SESSIONS} sessions with 16 buffers buffer 4097 parallelism 16"
        rv=$?
        if [ $rv -eq 0 ]
        then
                ok
...
run_offline()
{
        ...
        su - ${DB2USR} -c "db2 backup database ${SID} use TSM open $SESSIONS sessions with 16 buffers buffer 4097 parallelism 16"
        rv=$?
        if [ $rv -eq 0 ]
        then
                 ok
...
run_expiration()
{
        echo "================================================================"
        echo
        echo "Expiration for ${SID} started at $(date)"
        echo
        echo "================================================================"

        msg "Checking for Minimum Backups"
        NUMBKP=$(su - ${DB2USR} -c "db2adutl query full db ${SID}"  |grep -c "Oldest" |grep -v '[YOU HAVE NEW MAIL]')
        if [ $NUMBKP -lt 2 ]
        then
                echo There are less than 2 Online Backups....
                echo "================================================================"
                echo
                echo "Expiration for ${SID} Aborted at $(date)"
                echo
                echo "================================================================"
        else
                ok
                msg "Expiring old ONLINE Database Backups"
                su - ${DB2USR} -c "db2adutl DELETE FULL OLDER THAN ${ONLINE_DAYS} DAYS DATABASE ${SID} NODENAME ${HOST}_${SID}_ON without prompting"
                rv=$?
                if [ $rv -eq 0 ]
                then
                        ok
